...
//
	...
	public final int DICE_BAG = 15084;
	...
	public void addDices() {
		if (c.getItems().getItemCount(DICE_BAG) < 1 && c.getItems().getBankAmount(DICE_BAG) < 1) {
...
*/
	...
	public void startMovement(int x, int y, int height) {
		...
		if (c.getDungeoneering().Dung(c)) {
            c.sendMessage("You can't teleport out of Dungeoneering!");
            return;
        }
		//End
		if (c.duelStatus == 5) {
	...
	public void startTeleport(int x, int y, int height, String teleportType) {
		//Dungeoneering
		if (c.getDungeoneering().Dung(c)) {
            c.sendMessage("You cannot tele from a Dungeoneering!");
            return;
        }
		//End
		if (c.duelStatus == 5) {
		...
		if (!c.isDead && c.teleTimer == 0 && c.respawnTimer == -6) {
			...
			if(teleportType.equalsIgnoreCase("SpiritTree")) {
				c.startAnimation(1122);
				c.teleTimer = 9;
				c.gfx0(1228);
				c.teleEndAnimation = 1123;
			}
...
//End						
	...
	public void writePartlyQuestTab() {
		...
		c.getPA().sendFrame126("Combat Level: " + c.combatLevel + " ", 29165);
		c.getPA().sendFrame126("@gre@PBP [Prject Benelux Points]:", 29166); // quest
																		// title
		c.getPA().sendFrame126("@yel@" + c.pcPoints + " ", 29167);
		c.getPA().sendFrame126("@gre@Agility Points: ", 29168);
		c.getPA().sendFrame126("@yel@" + c.magePoints + " ", 29169);
	...
	public void writeFullQuestTab() {
		// quest journal title
		c.getPA().sendFrame126("Project Benelux", 29164); // quest title this
		c.getPA().sendFrame126("Your Combat level: ", 663);//this
		c.getPA().sendFrame126("Combat Level: " + c.combatLevel + " ", 29165);
		c.getPA().sendFrame126("@gre@PBP [Project Benelux Points]:", 29166); // quest
																		// title
		c.getPA().sendFrame126("@yel@" + c.pcPoints + " ", 29167);
		c.getPA().sendFrame126("@gre@Agility Points: ", 29168);
		c.getPA().sendFrame126("@yel@" + c.magePoints + " ", 29169);
