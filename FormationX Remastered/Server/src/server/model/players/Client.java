...
public class Client extends Player {
	...
	public Dungeoneering dungeoneering = new Dungeoneering(this);
	...
	public boolean checkEmpty(Client c) {
		if((getItems().freeSlots() == 28 && playerEquipment[playerHat] == -1) && summoningnpcid == 0 && (playerEquipment[playerCape] == -1) && (playerEquipment[playerAmulet] == -1) && (playerEquipment[playerChest] == -1) && (playerEquipment[playerShield] == -1) && (playerEquipment[playerLegs] == -1) && (playerEquipment[playerHands] == -1) && (playerEquipment[playerFeet] == -1) && (playerEquipment[playerWeapon] == -1)) {
			...
			sendMessage("Please bank your items before Dungeoneeringing!");
			return false;
		}
	}
	
	public boolean checkEmpty2(Client c) {
		if((getItems().freeSlots() == 28 && playerEquipment[playerHat] == -1) && summoningnpcid == 0 && (playerEquipment[playerCape] == -1) && (playerEquipment[playerAmulet] == -1) && (playerEquipment[playerChest] == -1) && (playerEquipment[playerShield] == -1) && (playerEquipment[playerLegs] == -1) && (playerEquipment[playerHands] == -1) && (playerEquipment[playerFeet] == -1) && (playerEquipment[playerWeapon] == -1)) {
			getDungeoneering().startfloor1(c);
			return true;
		} else {
...
public void handCannonDestory() {
	...
	public void initialize() {
		...
		sendMessage("Welcome to the Warlords of Hell Runescape Private Server");
		sendMessage("We dont have forums yet but please stay tuned!");
		if (lastsummon > 0) {
	...
	public void FetchDice()
	{
		...
		switch (cDice) {
		//Dice
			case 15096: rnd = Misc.random(19)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on a twenty-sided die."); break;
			case 15094: rnd = Misc.random(11)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on a twelve-sided die."); break;
			case 15092: rnd = Misc.random(9)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on a ten-sided die."); break;
			case 15090: rnd = Misc.random(7)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on an eight-sided die."); break;
			case 15100: rnd = Misc.random(3)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on a four-sided die."); break;
			case 15086: rnd = Misc.random(5)+1;	Message = ("rolled <col=18687>"+ rnd +"</col> on a six-sided die."); break;
			case 15088: rnd = Misc.random(11)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on two six-sided dice."); break;
			case 15098: rnd = Misc.random(99)+1; Message = ("rolled <col=18687>"+ rnd +"</col> on the percentile dice."); break;
		}
		sendMessage("You " + Message);
			if (clanDice){
				if (clanId >= 0) {
					Server.clanChat.messageToClan("Clan Chat channel-mate <col=18687>"+playerName+"</col> "+Message, clanId);
				}
			}
		cDice = 0;
	}
	...
	public void useDice(int itemId, boolean clan){
			...
			switch (itemId) {
				//Gfx's
				case 15086: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2072, absX, absY -1, 0, 0);
							break;
				case 15088: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2074, absX, absY -1, 0, 0);
							break;
				case 15090: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2071, absX, absY -1, 0, 0);
							break;
				case 15092: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2070, absX, absY -1, 0, 0);
							break;
				case 15094: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2073, absX, absY -1, 0, 0);
							break;
				case 15096: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2068, absX, absY -1, 0, 0);
							break;
				case 15098: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2075, absX, absY -1, 0, 0);
							break;
				case 15100: turnPlayerTo(absX, absY -1);
							getPA().createPlayersStillGfx(2069, absX, absY -1, 0, 0);
							break;
			}
		}
	...
	public synchronized boolean processPacket(Packet p) {
		...
		EventManager.getSingleton().addEvent(new Event() {
			public void execute(EventContainer c) {
				...
				r3 = Misc.random(6);
				if (r3 == 0) {
					sendMessage("<shad=15695415>[Warlords of Hell News</col>]: Please vote everyday, ::vote You get 10m!");
				} else if (r3 == 1) {           
					sendMessage("<shad=15695415>[Warlords of Hell News</col>]: Donations are appreciated thanks");		
				} else if (r3 == 2) { 
					sendMessage("<shad=15695415>[Warlords of Hell News</col>]: We don't have forums yet, but stay tuned!");
				} else if (r3 == 3) {
					sendMessage("<shad=15695415>[Warlords of Hell News</col>]: Feel free to ask any staff for help, as long as their not busy!");
				} else if (r3 == 4) {
					sendMessage("<shad=15695415>[Warlords of Hell News</col>]: Please report all bugs to a staff.");
				} else if (r3 == 5) {
					sendMessage("<shad=15695415>[Warlords of Hell News</col>]: Dont forget to click save in your quest tab!");
				}
			}
