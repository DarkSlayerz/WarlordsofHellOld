...
c.Lunar.Button(c, actionButtonId);
			...
			*/
			...
			if (c.dialogueAction == 106) {
				...
				if (c.teleAction == 1) {
					...
					c.getPA().spellTeleport(3299,9380,0);
					c.getPA().closeAllWindows();
				} else if (c.teleAction == 4) {
					c.dialogueId = 50;
		...
		if(System.currentTimeMillis() - c.lastEmote >= 1500) {
			...
			case 9180:
				...
				if(c.dialogueAction == 90) {
					c.getPA().removeAllWindows();
					c.sendMessage("Your current Dungeoneering level: "+c.getPA().getLevelForXP(c.playerXP[23])+"");
					c.sendMessage("Total amount of tokens: "+c.dungtokens+"");
				}
				if(c.dialogueAction == 101) {
			...
			case 9157://barrows tele to tunnels
				...
				} else if (c.dialogueAction == 167) {
					c.getPA().sendFrame126("www.warlordsofhell.co.cc", 12000);
					c.logout();
...
c.sendMessage("You do not have a npc currently spawned");
			...
			if(c.getItems().playerHasItem(995, 1000000)) {
				...
				c.sendMessage("You have run out money");
				c.dialogueAction = 0;
				c.getPA().removeAllWindows();
			}
