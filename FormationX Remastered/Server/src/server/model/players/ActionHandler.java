...
public class ActionHandler {
	...
	public void firstClickObject(int objectType, int obX, int obY) {
		...
		case 1294:
			c.objectDistance = 2;									
			c.stopMovement();
			c.sendMessage("You have been teleported to Skilling area.");
			c.getPA().startTeleport(2859, 3431, 0, "spiritTree");
			break;
		//Dungeoneering
		case 6553:
			if (!c.getDungeoneering().Dung(c)) {
			c.sendMessage("You cannot use theses objects, teleport home and re-do the Dungeon.");
			return;
			}
			if (c.absX  == 3234 && c.absY == 9323 && c.dungn == 0) {
			c.getPA().movePlayer(3234, 9325, 0);
			}
			if (c.absX  == 3260 && c.absY == 9329 && c.dungn == 3) {
				c.getPA().movePlayer(3261, 9331, 0);
			}
			if (c.absX  == 3261 && c.absY == 9329 && c.dungn == 3) {
				c.getPA().movePlayer(3260, 9331, 0);
			}
			if (c.absX  == 3246 && c.absY == 9333 && c.dungn == 6) {
					c.getPA().movePlayer(3244, 9333, 0);
			}
			if (c.absX  == 3246 && c.absY == 9334 && c.dungn == 6) {
				c.getPA().movePlayer(3244, 9334, 0);
		    }
			if (c.absX  == 3214 && c.absY == 9311 && c.dungn == 10) {
				c.getPA().movePlayer(3216, 9310, 0);
		    }
			if (c.absX  == 3214 && c.absY == 9310 && c.dungn == 10) {
				c.getPA().movePlayer(3216, 9311, 0);
		    }
			break;
		...
		case 6555:
			...
			if (c.absX  == 3233 && c.absY == 9323 && c.dungn == 0 ) {
				c.getPA().movePlayer(3233, 9325, 0);
			}
			break;
		...
		case 13616:
			...
			break;
		case 13617:
			c.getPA().movePlayer(2947, 3371, 0);
			c.sendMessage("Watch out, if u leave the bank. Other players might attack you!");
			break;
...
break;	
		...
		break;
				case 411:
			c.sendMessage("Everything works correctly!");
			if(c.altarPrayed == 0) {
				c.altarPrayed = 1;
		...
		break;
		case 6452:
			if (c.absX == 3304 && c.absY == 9376 || c.absX == 3304 && c.absY == 9375 ) {
		c.getPA().movePlayer(3305, 9376, 0);
		c.sendMessage("Prepare for the Corporeal Beast!");
				} else {
		c.autoRet = 0;
		...
		c.sendMessage("You teleported to the dragon lair.");
		break;
		case 13615:
		c.getPA().movePlayer(3210, 3424, 0);
		c.sendMessage("You teleported to the Tormented demon lair.");
		break;
		case 1738:
		...
		break;
		case 26428:
		      if (c.Zammy < 10 && c.absX == 2925 && c.absY == 5332) {
		       c.sendMessage("You need atleast 10 Zamorak KC to enter here!");
		       return;
		       }	
		...
		break;
		case 26425:
		      if (c.Band < 10 && c.absX == 2863 && c.absY == 5354) {
		       c.sendMessage("You need atleast 10 Bandos KC to enter here!");
		       return;
		       }	
		...
		break;
		case 26426:
		      if (c.Arma < 10 && c.absX == 2839 && c.absY == 5295) {
		       c.sendMessage("You need atleast 10 Armadyl KC to enter here!");
		       return;
		       }		       
		...
		break;
		case 26427:
		      if (c.Sara < 10 && c.absX == 2908 && c.absY == 5265) {
		       c.sendMessage("You need atleast 10 Saradomin KC to enter here!");
		       return;
		       }	
	...
	case 1814:
		...
		c.getPA().startLeverTeleport(3154, 3923, 0, "lever");
            }		
		break;
...
c.sendMessage("Your hitpoints has been restored, and you've also been un teleblocked.");
c.teleBlockLength = 0;
 } else {
c.sendMessage("You already have full hitpoints.");
		...
		case 1815:
			c.getPA().startLeverTeleport(Config.EDGEVILLE_X, Config.EDGEVILLE_Y, 0, "lever");
		break;
	...
	c.fishitem = -1;
		...
		switch(npcType) {
				...
				break;
				case 2830:
				c.getDH().sendDialogues(165, npcType);
				break;
								case 8206:
...
break;
	...
	c.fishitem = -1;
		...
		break;
			...
			case 924:
				c.getShops().openShop(20);
				c.sendMessage("You have " + c.ProjectBeneluxPoints + " Project Benelux Points. Gain these from Slayer/Pc/Pk/Duel arena!");
				c.sendMessage("<shad=15369497>Ring of Vigour makes ur spec restore faster!");
			break;
			case 3789:
				c.getShops().openShop(18);
				c.sendMessage("You have " + c.ProjectBeneluxPoints + " FXP. Gain these from Slayer/Pc/Pk/Duel arena!");
				c.sendMessage("<shad=15369497>Ring of Vigour makes ur spec restore faster!");
			break;
			case 1:
			case 9:
