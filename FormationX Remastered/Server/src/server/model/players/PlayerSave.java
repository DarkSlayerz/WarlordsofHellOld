		...
		while (EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("leverClicked")) {
						p.leverClicked = Boolean.parseBoolean(token2);
					//break;
					...
					} else if (token.equals("dungn")) {
						p.dungn = Integer.parseInt(token2);
					} else if (token.equals("dungtokens")) {
						p.dungtokens = Integer.parseInt(token2);
	...
	characterfile.newLine();
			for (int i = 0; i < p.lastConnectedFrom.size(); i++) {
				...
				characterfile.newLine();
						...
						characterfile.write("leverClicked = ", 0, 15);
			characterfile.write(Boolean.toString(p.leverClicked), 0, Boolean.toString(p.leverClicked).length());
			characterfile.newLine();
