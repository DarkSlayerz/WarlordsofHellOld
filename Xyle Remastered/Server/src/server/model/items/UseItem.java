...
import server.model.players.Summoning.Familiar;
...
public class UseItem {
	...
	public static void ItemonItem(Client c, int itemUsed, int useWith, int usedWithSlot) {
		if(itemUsed == 12435) {
			if(c.getSummoning().summonedFamiliar != null) {
				if(c.getSummoning().summonedFamiliar == Familiar.PACK_YAK) {
					c.getSummoning().useSpecial(useWith, usedWithSlot, c);
				}
			}
		}
