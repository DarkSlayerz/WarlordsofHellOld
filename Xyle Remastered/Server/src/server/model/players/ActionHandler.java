...
public class ActionHandler {
	...
	public void firstClickObject(int objectType, int obX, int obY) {
		...
		c.getPA().walkTo(0, c.absY <= c.objectY ? +3 : -3);
		break;
	    case 10595:
			...
			if (System.currentTimeMillis() - c.lastAction < 500)
				return;
					if (c.objectX == 3090 && c.objectY == 3956) {
							   new Object(2421, c.objectX, c.objectY, c.heightLevel, 1, 2, 5959, 200);
			...
			c.startAnimation(c.getCombat().getWepAnim(c.getItems().getItemName(c.playerEquipment[c.playerWeapon]).toLowerCase()));
				if (Misc.random(1) == 1) {
						...
						if (c.objectX == 3158 && c.objectY == 3951) {
                                new Object(734, c.objectX, c.objectY, c.heightLevel, 1, 10, 733, 200);
                        } else {
                                new Object(734, c.objectX, c.objectY, c.heightLevel, 0, 10, 733, 200);//needa fix position of obj 734 at mage arena easy shit tho
                        }
					} else {
							c.sendMessage("You fail to slash the webs.");
							c.lastAction = System.currentTimeMillis() + 2000;
                        }
			} else if (!c.wearingSharpWeapon()) {
