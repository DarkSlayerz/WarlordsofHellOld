	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		switch(packetType) {
			...
			**/
			...
			if(Server.npcHandler.npcs[c.npcIndex].MaxHP == 0 || Server.npcHandler.npcs[c.npcIndex].npcType == 944 || Server.npcHandler.npcs[c.npcIndex].summonedFor > 0){
			...
			case FIRST_CLICK:
				...
				if(c.goodDistance(Server.npcHandler.npcs[c.npcClickIndex].getX(), Server.npcHandler.npcs[c.npcClickIndex].getY(), c.getX(), c.getY(), 1)) {
					...
					if(Server.npcHandler.npcs[c.npcClickIndex].summonedFor > 0) {
						if(Server.npcHandler.npcs[c.npcClickIndex].summonedFor == c.getId()) {
							/*if(c.getSummoning().summonedFamiliar.familiarType == FamiliarType.BOB) {
							c.getDH().sendDialogues(201, npcType);
							}
							if(c.getSummoning().summonedFamiliar.familiarType == FamiliarType.TELEPORTER) {
				...
				if(c.goodDistance(Server.npcHandler.npcs[c.npcClickIndex].getX(), Server.npcHandler.npcs[c.npcClickIndex].getY(), c.getX(), c.getY(), 1)) {
					...
					if(Server.npcHandler.npcs[c.npcClickIndex].summonedFor > 0) {
						if(Server.npcHandler.npcs[c.npcClickIndex].summonedFor == c.getId()) {
							...
							c.resetWalkingQueue();
							return;
						} else if(Server.npcHandler.npcs[c.npcClickIndex].summonedFor != -1) {
							c.sendMessage("This is not your familiar!");
							c.resetWalkingQueue();
							return;
						}
