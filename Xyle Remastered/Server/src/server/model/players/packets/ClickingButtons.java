...
import server.model.players.Prayers;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		Prayer prayer = Prayer.forId(actionButtonId);
		if(prayer != null) {
			Prayers.togglePrayer(c, prayer, false);
		}
		Prayer quickPrayer = Prayer.quickId(actionButtonId);
		if(quickPrayer != null) {
			Prayers.togglePrayer(c, quickPrayer, true);
		}
		switch (actionButtonId){
			case 58152:
				if(c.getQuickPrayers().size() > 0) {
					c.getActivePrayers().clear();
					c.headIcon = -1;
					c.getPA().requestUpdates();
					Prayers.refreshPrayers(c, false);
					for(int i = 0; i < c.getQuickPrayers().size(); i++) {
						Prayers.togglePrayer(c, c.getQuickPrayers().get(i), false);
					}
					c.sendMessage(":prayclicked:");
				} else {
					c.sendMessage("Try setting some quick prayers, before you toggle them!");
				}
				break;
			case 58153:
				c.getActivePrayers().clear();
				...
				break;
			case 58154:
				Prayers.refreshPrayers(c, true);
				c.setSidebarInterface(5, c.isOnCurses() ? 23500 : 17200);
				break;
			case 67079:
				c.setSidebarInterface(5, c.isOnCurses() ? 22500 : 5608);
				break;
			case 150:
			...
			case 9181:
				if (c.usingGlory)
					...
					c.getPA().specialTeleport(3149, 3660, 0, 2);
				else if (c.getPA().dialogueAction(2)) 
		...
		if (c.isAutoButton(actionButtonId))
			...
			c.getSummoning().handleButtonClick(actionButtonId);
	}
