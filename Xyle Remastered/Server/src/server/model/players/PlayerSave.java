		...
		while(EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
					...
					} else if (token.equals("clanchat")) {
						p.savedClan = token2;
					} else if (token.equals("character-posy")) {
						p.teleportToY = (Integer.parseInt(token2) <= 0 ? 3424 : Integer.parseInt(token2));
					} else if (token.equals("specTimer")) {
						p.specRestoreTimer = Integer.parseInt(token2);
						...
						p.joinTime = Long.parseLong(token2);//Starter save	
						...
						p.constitution = Integer.parseInt(token2);
				...
				case 10:
					...
					break;
				case 11:
					if (token.equals("character-qp")) {
						Prayer quickPrayer = Prayer.quickId(Integer.parseInt(token3[1]));
						if(quickPrayer != null) {
							p.getQuickPrayers().add(quickPrayer);
						}
					}
					break;
				}
				...
				} else if (line.equals("[BOB]")) {		ReadMode = 10;
				} else if (line.equals("[QP]")) {		ReadMode = 11;
	...
	**/
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("clanchat = ", 0, 11);
			characterfile.write(p.savedClan, 0, p.savedClan.length());
			characterfile.newLine();
			characterfile.write("specTimer = ", 0, 12);
			characterfile.write(Integer.toString(p.specRestoreTimer), 0, Integer.toString(p.specRestoreTimer).length());
			characterfile.newLine();
			characterfile.write("familiar = ", 0, 11);
			characterfile.write(Integer.toString(p.getSummoning().summonedFamiliar == null ? -1 : p.getSummoning().summonedFamiliar.pouchId), 0, Integer.toString(p.getSummoning().summonedFamiliar == null ? -1 : p.getSummoning().summonedFamiliar.pouchId).length());
			characterfile.newLine();
		...
		/*BANK*/
			...
			if(p.getSummoning().burdenedItems != null) {
				for (int i = 0; i < p.getSummoning().burdenedItems.length; i++) {
					if (p.getSummoning().burdenedItems[i] > 0) {
						...
						characterfile.write(Integer.toString(i), 0, Integer.toString(i).length());
						characterfile.write("	", 0, 1);
						characterfile.write(Integer.toString(p.getSummoning().burdenedItems[i]), 0, Integer.toString(p.getSummoning().burdenedItems[i]).length());
						characterfile.newLine();
					}
				}
			}
			...
			characterfile.newLine();
			if(p.getQuickPrayers() != null) {
				for (int i = 0; i < p.getQuickPrayers().size(); i++) {
					characterfile.write("character-qp = ", 0, 15);
					characterfile.write(Integer.toString(i), 0, Integer.toString(i).length());
					characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.getQuickPrayers().get(i).getQuickPrayerId()), 0, Integer.toString(p.getQuickPrayers().get(i).getQuickConfiguration()).length());
					characterfile.newLine();
				}
			}
			characterfile.newLine();
