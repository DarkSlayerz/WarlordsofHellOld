...
public class TradeAndDuel{
	...
	**/
	...
	public void requestTrade(int id){
		if (PlayerHandler.updateRunning) {
			c.sendMessage("You cannot do this while an update is taking place!");
			return;
		}
		try {
			...
			if(!c.inTrade && o.tradeRequested && o.tradeWith == c.playerId) {
				if (!o.isBanking || !c.isBanking) {
					c.getTradeAndDuel().openTrade();
					o.getTradeAndDuel().openTrade();
				} else {
					c.sendMessage("That player is busy at the moment.");
				}
			} else if(!c.inTrade) {
				if (!o.inTrade) {
	...
	public void openTrade() {
		...
		if(o == null || o.isBanking || c.isBanking) {
			return;
		}
		c.inTrade = true;
		c.canOffer = true;
	...
	public boolean tradeItem(int itemID, int fromSlot, int amount) {
		Client o = (Client) Server.playerHandler.players[c.tradeWith];
		if(o == null || !c.getItems().playerHasItem(itemID, 1, fromSlot)) {
			return false;
		}
	...
	**/
	...
	public boolean stakeItem(int itemID, int fromSlot, int amount) {
		if (!c.getItems().playerHasItem(itemID, 1, fromSlot))
			return false;
		for (int i : Config.ITEM_TRADEABLE) {
			if(i == itemID) {
	...
	public void confirmDuel() {
		Client o = (Client) Server.playerHandler.players[c.duelingWith];
		if(o == null || (c.getSummoning().summonedFamiliar != null || o.getSummoning().summonedFamiliar != null)) {
			if(c.getSummoning().summonedFamiliar != null || o.getSummoning().summonedFamiliar != null) {
				c.sendMessage("You cannot duel with familiars!");
				o.sendMessage("You cannot duel with familiars!");
			}
			declineDuel();
			return;
		}
